...
class ReporteRepository(BaseRepository):
    """Repositorio para la persistencia de reportes académicos."""

    def obtener_todos(self) -> List[Dict[str, Any]]:
        return self._json_manager.leer_archivo(self._ruta_archivo)

    def obtener_por_id(self, id: Any) -> Optional[Dict[str, Any]]:
        for reporte in self.obtener_todos():
            if isinstance(reporte, dict) and reporte.get("id") == id:
                return reporte
        return None
    ...
    def buscar_por_usuario(self, usuario_id: Any) -> List[Dict[str, Any]]:
        return [
            reporte
            for reporte in self.obtener_todos()
            if isinstance(reporte, dict) and reporte.get("usuario_id") == usuario_id
        ]
